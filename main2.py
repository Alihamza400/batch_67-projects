...
if(status=='male'):
    ...
    st.success('Female');
Hobby = st.selectbox("Hobbies:",['Book Reading','Problem Solving',"Exploring new Thing"]);
st.write("Hobby is = ",Hobby);
Hobbies = st.multiselect("Hobbies = ",['Book Reading','Problem Solving','Newspaper Reading']);
st.write("Your Selected :",len(Hobbies),Hobbies,'Hobbies');
st.button("Click me for no reason");
if(st.button('About')):
