import streamlit as st
import pandas as pd
# st.write("Pakistan is an islamic state");
# st.header("Islam is an only solution of the world problem");
# st.header("We are independent muslim");
st.title("Pakistan is only muslim atomic state");
st.write("Speak and write right words");
st.text("Text of 1973 constituition eloquence");
st.markdown("Muslim are fearless");
st.latex("e4-4xy");
st.header("World Problems");
st.subheader("Climate Change");
st.code("Write code here");
st.title("Data Display")
st.title("Display Media")
st.video("https://youtu.be/_OVnXw2ldog?si=2qsVAd3WdTUAlRVH")
st.image("data:image/png;base64,")
# st.audio("urdu.mp3")
df : pd.DataFrame = pd.DataFrame({'col1': [1,2,3],'col2':list('abc')})
df  # 👈 Draw the dataframe
# fd:pd.DataFrame = pd.DataFrame({'col1':[1,2,3,4,5],'col2':list('HAMZA')})
# fd


x : int = 100

'x', x  # 👈 Draw the string 'x' and then the value of x